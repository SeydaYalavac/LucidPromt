...
export function TrendingView({ initialQuery = "" }: { initialQuery?: string }) {
  ...
  return <div className="mx-auto min-h-screen max-w-[1440px] px-5 pb-28 pt-32 sm:px-8 sm:pt-40">
    ...
    <div className="mt-10 grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_300px]">
      ...
      <aside className="editorial-card order-first rounded-2xl border p-6 lg:sticky lg:top-28 lg:order-last" aria-label={t("filters.aria")}><div className="flex items-center gap-2"><SlidersHorizontal size={17} className="text-white/55" /><h2 className="text-sm font-medium text-white">{t("filters.title")}</h2></div><label className="mt-6 block text-xs uppercase tracking-widest text-[#8B8B93]">{t("filters.search")}<input value={query} onChange={(event) => { setQuery(event.target.value); setVisibleCount(8); }} className="mt-2 h-12 w-full rounded-xl border border-white/10 bg-white/[0.035] px-4 text-sm text-white outline-none focus:border-white/50 focus-visible:ring-2 focus-visible:ring-white/70" placeholder={t("filters.placeholder")} /></label><label className="mt-5 block text-xs uppercase tracking-widest text-[#8B8B93]">{t("filters.category")}<select value={activeCategory} onChange={(event) => { setCategory(event.target.value); setVisibleCount(8); }} className="mt-2 h-12 w-full rounded-xl border border-white/10 bg-[#111114] px-4 text-sm text-white outline-none focus:border-white/50 focus-visible:ring-2 focus-visible:ring-white/70">{categories.map((item) => <option key={item} value={item}>{localeCategoryLabel(item, locale)}</option>)}</select></label><label className="mt-5 block text-xs uppercase tracking-widest text-[#8B8B93]">{t("filters.order")}<select value={sort} onChange={(event) => { setSort(event.target.value as SortMode); setVisibleCount(8); }} className="mt-2 h-12 w-full rounded-xl border border-white/10 bg-[#111114] px-4 text-sm text-white outline-none focus:border-white/50 focus-visible:ring-2 focus-visible:ring-white/70"><option value="score">{t("filters.highest")}</option><option value="velocity">{t("filters.fastest")}</option><option value="newest">{t("filters.newest")}</option></select></label><label className="mt-6 flex min-h-11 cursor-pointer items-center justify-between gap-3 rounded-xl border border-white/[0.08] px-4 text-sm text-[#C4C4CA] focus-within:outline-2 focus-within:outline-offset-2 focus-within:outline-white"><span>{t("filters.saved")}</span><input type="checkbox" checked={savedOnly} onChange={(event) => { setSavedOnly(event.target.checked); setVisibleCount(8); }} className="h-4 w-4 accent-white" /></label><button type="button" onClick={() => { setQuery(""); setCategory("All"); setSort("score"); setSavedOnly(false); setVisibleCount(8); }} className="mt-4 min-h-11 w-full text-xs uppercase tracking-widest text-white/45 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white">{t("filters.reset")}</button></aside>
    </div>
  </div>;
}
