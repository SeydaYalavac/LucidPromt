...
export function NewsCard({ trend, featured = false, rank }: { trend: Trend; featured?: boolean; rank?: number }) {
  ...
  return <article className={`group relative flex h-full flex-col overflow-hidden rounded-[1.75rem] border border-white/[0.08] bg-[#0B0B0D] p-6 transition-colors hover:border-white/20 sm:p-7 ${featured ? "min-h-[440px] sm:p-10" : "min-h-[270px]"}`}>
