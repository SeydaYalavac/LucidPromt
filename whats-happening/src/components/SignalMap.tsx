...
function DevelopmentDetail({ name, activity, point, onSelect }: { name: string; activity: CountryActivity | null; point: MapDevelopmentPoint | null; onSelect: (id: string) => void }) {
  if (!activity || !point) {
    return <aside className="rounded-[1.75rem] border border-white/[0.12] bg-[#0B0B0D] p-6 sm:p-7" aria-live="polite">
      <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/45">Selected country</p>
      <h2 className="mt-2 text-3xl font-medium tracking-[-0.04em] text-white">{name}</h2>
      <p className="mt-5 text-sm leading-6 text-[#A8A8AF]">No verified, country-attributed AI developments match the current filters. The country remains selectable, but it stays unlit until source evidence arrives.</p>
    </aside>;
  }
  ...
  return <aside className="rounded-[1.75rem] border border-white/[0.12] bg-[#0B0B0D] p-6 sm:p-7" aria-live="polite">
    <div className="flex items-start justify-between gap-5">
      <div>
        <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-white/45">Verified development · {activity.country.name}</p>
    ...
    <p className="mt-5 text-sm leading-6 text-[#B7B7BD]">{point.signal_summary || point.trend_summary || "The official source recorded this AI topic without a longer summary."}</p>
    <dl className="mt-6 space-y-4 border-y border-white/[0.09] py-5 text-sm">
      <div className="grid grid-cols-[5rem_1fr] gap-3"><dt className="text-white/40">Observed</dt><dd className="text-white">{formatUtc(point.observed_at || point.published_at)}</dd></div>
      <div className="grid grid-cols-[5rem_1fr] gap-3"><dt className="text-white/40">Geography</dt><dd className="leading-6 text-white">{point.geographic_evidence}</dd></div>
      <div className="grid grid-cols-[5rem_1fr] gap-3"><dt className="text-white/40">Topic</dt><dd className="text-white">{point.trend_title}</dd></div>
    </dl>
    <div className="mt-6 grid gap-3 sm:grid-cols-2">
      <a href={point.source_url} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-white px-5 text-sm font-semibold text-black hover:bg-[#E7E7E9] focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-white">Open {point.provider_label} source <ExternalLink size={15} /></a>
      <Link href={`/trend/${point.trend_slug}`} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-white/15 px-5 text-sm font-medium text-white hover:bg-white/[0.06] focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-white">Read trend article <ArrowRight size={15} /></Link>
    </div>
    {activity.developments.length > 1 && <div className="mt-8 border-t border-white/[0.09] pt-6">
      <h3 className="text-sm font-medium text-white">Developments in this cluster</h3>
    ...
    <p className="mt-7 border-t border-white/[0.08] pt-5 text-xs leading-5 text-white/40">Country-level source evidence does not establish an exact city, event origin, cause, or forecast.</p>
  </aside>;
}
...
export function SignalMap() {
  ...
  const effectiveCountryName = selectedCountryName || (country !== "All countries" ? country : activities[0] ? atlasName(activities[0].country) : "Choose a country");
  ...
  return <div className="mx-auto min-h-screen max-w-[1600px] px-4 pb-20 pt-28 sm:px-6 sm:pt-32">
    <header className="grid gap-6 border-b border-white/[0.08] pb-8 lg:grid-cols-[1fr_34rem] lg:items-end">
      <div>
        ...
        <h1 className="mt-3 max-w-4xl text-balance text-[clamp(3rem,7vw,6.5rem)] font-medium leading-[0.9] tracking-[-0.065em] text-white">Global AI developments, mapped.</h1>
      </div>
      <div className="max-w-[60ch] lg:pb-1">
        <p className="text-pretty text-base leading-7 text-[#A8A8AF]">Select any country. Verified developments light up as clustered points, with the source and reporting attached.</p>
        {data?.mode === "demo" && <p className="mt-4 inline-flex rounded-full border border-white/20 bg-white/[0.08] px-3 py-1.5 font-mono text-[10px] uppercase tracking-[0.16em] text-white">Demo data · interaction preview</p>}
        {data && <p className="mt-4 font-mono text-xs uppercase tracking-[0.12em] text-white/45"><span className="tabular-nums text-white/75">{data.coverage.countries_with_evidence}</span> lit markets · <span className="tabular-nums text-white/75">{data.coverage.attributed_evidence_count}</span> developments · {formatUtc(data.coverage.as_of)}</p>}
      </div>
    </header>

    <section className="mt-7 grid gap-3 rounded-[1.5rem] border border-white/[0.1] bg-[#0B0B0D] p-3 sm:grid-cols-2 xl:grid-cols-[minmax(230px,1.4fr)_repeat(4,minmax(140px,0.7fr))]" aria-label="Map filters">
      <label className="flex min-h-12 items-center gap-3 rounded-xl border border-white/[0.08] bg-[#08090A] px-4"><Search size={16} className="text-white/40" aria-hidden="true" /><span className="sr-only">Search developments</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search AI developments" className="min-w-0 flex-1 bg-transparent text-sm text-white outline-none placeholder:text-white/35" />{query && <button type="button" onClick={() => setQuery("")} aria-label="Clear map search" className="flex min-h-11 min-w-11 items-center justify-center text-white/40 hover:text-white"><X size={15} /></button>}</label>
      <label className="sr-only" htmlFor="map-country">Country</label><select id="map-country" value={country} onChange={(event) => { const nextCountry = event.target.value; setCountry(nextCountry); if (nextCountry === "All countries") { setSelectedCountryName(null); setSelectedPointId(null); } else { chooseCountry(nextCountry); } }} className="min-h-12 min-w-0 rounded-xl border border-white/[0.08] bg-[#08090A] px-3 text-sm text-white outline-none focus:border-white/50"><option>All countries</option>{countryOptions.map((item) => <option key={item}>{item}</option>)}</select>
      <label className="sr-only" htmlFor="map-region">Region</label><select id="map-region" value={region} onChange={(event) => { setRegion(event.target.value); setCountry("All countries"); setSelectedCountryName(null); }} className="min-h-12 min-w-0 rounded-xl border border-white/[0.08] bg-[#08090A] px-3 text-sm text-white outline-none focus:border-white/50">{regions.map((item) => <option key={item}>{item}</option>)}</select>
      <label className="sr-only" htmlFor="map-category">AI topic</label><select id="map-category" value={category} onChange={(event) => setCategory(event.target.value)} className="min-h-12 min-w-0 rounded-xl border border-white/[0.08] bg-[#08090A] px-3 text-sm text-white outline-none focus:border-white/50">{categories.map((item) => <option key={item}>{item}</option>)}</select>
      <label className="sr-only" htmlFor="map-time">Time window</label><select id="map-time" value={timeWindow} onChange={(event) => setTimeWindow(event.target.value as TimeWindow)} className="min-h-12 min-w-0 rounded-xl border border-white/[0.08] bg-[#08090A] px-3 text-sm text-white outline-none focus:border-white/50"><option value="24">Last 24 hours</option><option value="48">Last 48 hours</option></select>
    </section>

    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_410px] xl:items-start">
      <section className="relative min-h-[560px] overflow-hidden rounded-[2rem] border border-white/[0.12] bg-[#08090A] sm:min-h-[660px]" aria-label="Interactive world AI activity map" aria-describedby="map-evidence-limit">
        <div className="absolute bottom-6 right-5 z-20 flex flex-col gap-2"><button type="button" onClick={() => zoom(0.5)} className="flex min-h-11 min-w-11 items-center justify-center rounded-full border border-white/15 bg-[#0B0B0D]/95 text-white hover:bg-[#17171B] focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-white" aria-label="Zoom map in"><Plus size={17} /></button><button type="button" onClick={() => zoom(-0.5)} className="flex min-h-11 min-w-11 items-center justify-center rounded-full border border-white/15 bg-[#0B0B0D]/95 text-white hover:bg-[#17171B] focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-white" aria-label="Zoom map out"><Minus size={17} /></button><button type="button" onClick={reset} className="flex min-h-11 min-w-11 items-center justify-center rounded-full border border-white/15 bg-[#0B0B0D]/95 text-white hover:bg-[#17171B] focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-white" aria-label="Reset map position"><LocateFixed size={17} /></button></div>

        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} role="group" aria-label={`World map with ${activities.length} markets showing verified AI developments`} tabIndex={0} onKeyDown={(event) => { if (event.key === "+" || event.key === "=") zoom(0.5); if (event.key === "-") zoom(-0.5); if (event.key === "0") reset(); if (event.key === "ArrowLeft") setTransform((value) => ({ ...value, x: value.x + 24 })); if (event.key === "ArrowRight") setTransform((value) => ({ ...value, x: value.x - 24 })); if (event.key === "ArrowUp") setTransform((value) => ({ ...value, y: value.y + 24 })); if (event.key === "ArrowDown") setTransform((value) => ({ ...value, y: value.y - 24 })); }} onWheel={(event) => { event.preventDefault(); zoom(event.deltaY > 0 ? -0.2 : 0.2); }} onPointerDown={(event) => { event.currentTarget.setPointerCapture(event.pointerId); dragStart.current = { x: event.clientX, y: event.clientY, originX: transform.x, originY: transform.y }; }} onPointerMove={(event) => { if (!dragStart.current) return; const ratio = WIDTH / event.currentTarget.getBoundingClientRect().width; setTransform((value) => ({ ...value, x: dragStart.current!.originX + (event.clientX - dragStart.current!.x) * ratio, y: dragStart.current!.originY + (event.clientY - dragStart.current!.y) * ratio })); }} onPointerUp={() => { dragStart.current = null; }} onPointerCancel={() => { dragStart.current = null; }} className="absolute inset-0 h-full w-full touch-none cursor-grab focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-white/70">
          <g transform={`translate(${transform.x} ${transform.y}) scale(${transform.scale})`}>
            ...
            {atlasCountries.features.map((atlasCountry, index) => {
              ...
              return <path key={atlasCountry.id == null ? index : String(atlasCountry.id)} d={path(atlasCountry) || undefined} role="button" tabIndex={0} aria-label={`${name}${active ? `, ${activityByAtlasName.get(name)?.evidence_count} verified developments` : ", no verified developments"}`} onClick={(event) => { event.stopPropagation(); chooseCountry(name); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); chooseCountry(name); } }} fill={active ? "#23262B" : selected ? "#1A1C20" : "#111317"} stroke={selected ? "#F5F5F5" : active ? "rgba(245,245,245,.48)" : "rgba(255,255,255,.15)"} strokeWidth={(selected ? 1.2 : 0.55) / transform.scale} className="cursor-pointer outline-none" />;
            })}
            {activities.map((activity) => {
              ...
              return <g key={activity.country.code} transform={`translate(${point[0]} ${point[1]})`} role="button" aria-label={`${activity.country.name}: cluster of ${activity.evidence_count} verified AI developments. Open latest development.`} tabIndex={0} onClick={(event) => { event.stopPropagation(); chooseCountry(atlasName(activity.country), activity); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); chooseCountry(atlasName(activity.country), activity); } }} className="cursor-pointer outline-none focus-visible:[&_circle]:stroke-white">
        ...
        <div className="absolute bottom-5 left-5 z-10 max-w-[calc(100%-6rem)] rounded-xl border border-white/[0.1] bg-[#0B0B0D]/92 px-4 py-3 font-mono uppercase backdrop-blur-xl"><p className="text-[10px] tracking-widest text-white/55">Point = verified development · number = cluster · drag to pan</p><p className="mt-1.5 text-[8px] tracking-wider text-white/25">Boundary data: Natural Earth via world-atlas · public domain</p></div>
        {error && <div className="absolute inset-x-5 top-6 z-30 sm:inset-x-auto sm:left-6 sm:w-[470px]"><TrendUnavailable message="The map remains available, but source-attributed activity could not be loaded." /></div>}
        {!error && !isLoading && !activities.length && <div className="absolute left-1/2 top-1/2 z-10 w-[min(90%,440px)] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-white/[0.1] bg-[#0B0B0D]/95 p-6 text-center text-sm leading-6 text-[#A8A8AF]">No verified, country-attributed AI developments match these filters. Every country remains selectable; places without current evidence stay unlit.</div>}
    ...
    <section className="mt-14 border-t border-white/[0.08] pt-10" aria-labelledby="activity-list-heading">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><p className="font-mono text-xs uppercase tracking-[0.16em] text-white/40">Accessible map alternative</p><h2 id="activity-list-heading" className="mt-2 text-3xl font-medium tracking-[-0.04em] text-white">Browse every verified development.</h2></div>
        <p className="max-w-md text-sm leading-6 text-[#8F8F98]">The list uses the same country, region, topic and time filters as the map.</p>
      </div>
      <div className="mt-7 divide-y divide-white/[0.08] border-y border-white/[0.08]">
        {activities.map((activity) => <details key={activity.country.code} className="group py-1">
          <summary className="flex min-h-16 cursor-pointer list-none items-center justify-between gap-4 py-4 focus-visible:outline-2 focus-visible:outline-offset-3 focus-visible:outline-white">
            ...
            <span className="flex shrink-0 items-center gap-3"><span className="font-mono text-xs tabular-nums text-white/55">{activity.evidence_count} developments</span><Plus size={16} className="text-white/45 group-open:rotate-45" aria-hidden="true" /></span>
          </summary>
          <div className="grid gap-4 pb-7 sm:grid-cols-2 xl:grid-cols-3">
            {activity.developments.map((point) => <article key={point.id} className="rounded-2xl border border-white/[0.1] bg-[#0B0B0D] p-5">
              <p className="font-mono text-[10px] uppercase tracking-wider text-white/35">{point.category} · {formatUtc(point.observed_at)}</p>
              ...
              <p className="mt-4 text-xs leading-5 text-white/40">{point.country.name} · country-level evidence</p>
              <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2"><a href={point.source_url} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center gap-1.5 text-xs font-medium text-white hover:text-white/70">{point.provider_label}<ExternalLink size={12} /><span className="sr-only"> source for {point.source_title}, opens in a new tab</span></a><Link href={`/trend/${point.trend_slug}`} className="inline-flex min-h-11 items-center gap-1.5 text-xs font-medium text-[#C4C4CA] hover:text-white">Trend article<ArrowRight size={12} /></Link></div>
            </article>)}
          </div>
        </details>)}
      </div>
      <p id="map-evidence-limit" className="mt-6 max-w-[70ch] text-xs leading-5 text-white/40">Only current AI developments with source-attributed country evidence are lit. Country-level points use the market coordinate and never imply city precision or origin. The map refreshes from the same 10-minute ingestion cycle as the live trend feed.</p>
