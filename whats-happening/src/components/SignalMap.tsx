  ...
  return <div className="mx-auto min-h-screen max-w-[1600px] px-4 pb-20 pt-28 sm:px-6 sm:pt-32">
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_410px] xl:items-start">
      <section className="editorial-card relative min-h-[560px] overflow-hidden rounded-[2rem] border sm:min-h-[660px]" aria-label={l("Interactive world AI activity map", "Etkileşimli dünya yapay zeka etkinlik haritası")} aria-describedby="map-evidence-limit">
        ...
        {!error && !isLoading && !activities.length && <div className="editorial-card absolute left-1/2 top-1/2 z-10 w-[min(90%,440px)] -translate-x-1/2 -translate-y-1/2 rounded-2xl border p-6 text-center text-sm leading-6 text-[#A8A8AF]">{l("No verified, country-attributed AI developments match these filters. Every country remains selectable; places without current evidence stay unlit.", "Bu filtrelerle eşleşen doğrulanmış, ülkeye atfedilmiş yapay zeka gelişmesi yok. Her ülke seçilebilir kalır; güncel kanıt olmayan yerler ışıklandırılmaz.")}</div>}
