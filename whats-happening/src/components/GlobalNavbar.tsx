...
import { getSignupHref } from "@/lib/auth-redirect";
...
export function GlobalNavbar({ showPrimaryAuthAction = true }: GlobalNavbarProps) {
  ...
  const { t } = useLocale();
  const signupHref = getSignupHref(pathname);
        ...
        <div className="mx-auto flex max-w-[1440px] items-center justify-between gap-5">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            {!isLoading && !session && <>
              ...
              {showPrimaryAuthAction && <Link href={signupHref} onClick={() => captureProductEvent("signup_cta_clicked", { source: "global_nav" })} className="primary-action hidden text-[11px] uppercase tracking-[0.12em] sm:flex">{t("nav.signUp")}</Link>}
            </>}
