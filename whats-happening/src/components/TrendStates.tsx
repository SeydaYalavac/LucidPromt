...
export function TrendLoading({ compact = false }: { compact?: boolean }) {
  const { t } = useLocale();
  return <div className={`animate-pulse rounded-[1.75rem] border border-white/[0.05] bg-[#0D0D0F] ${compact ? "h-40" : "h-80"}`} aria-label={t("state.loading")} />;
}
...
export function TrendEmpty({ message }: { message?: string }) {
  const { t } = useLocale();
  return <div className="rounded-[1.75rem] border border-dashed border-white/10 px-8 py-16 text-center"><Radio size={20} className="mx-auto text-white/35" /><p className="mt-4 text-sm text-[#9B9BA4]">{message || t("state.empty")}</p></div>;
}
