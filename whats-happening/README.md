...
`Read verified Auth contact candidates` workflow'u yalnızca manuel çalışır ve son 1-744 saatteki doğrulanmış kayıtları denetler. Yalnız güvenilir adı bulunan ilk uygun kayıt mode `0600` tek kullanımlık runner dosyasına geçer. Aynı çalışmada yönetilen posta kutusu tam adres ve `What were you hoping to track?` konusu ile kontrol edilir; dosya başarıda ve hatada yok edilir. E-posta ve ad hiçbir HTTP yanıtına, client paketine, loga, GitHub özetine veya artifact'a yazılmaz. Log yalnız taranan, dışlanan, aktarılan, tekrar bulunan, hazır ve gönderilen kayıt sayılarını gösterir. Gönderim varsayılan olarak kapalıdır; açıldığında yalnız önceden onaylanmış `cycle5_v1` notu kullanılabilir ve platformun günlük 25 e-posta sınırı değişmez.
